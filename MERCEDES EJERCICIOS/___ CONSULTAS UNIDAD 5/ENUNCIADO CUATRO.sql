/******
    ...
    **/ 
       ...
       SELECT   
          ...
          FROM EMPLEADOS AS ES INNER JOIN   EMPLEADOS AS E
                                ON ES.ID_EMPLEADO=E.SUPERVISOR /* (O,N)*/
    ...
    /* DE OTRA FORMA**/
      ...
      SELECT  
              (
                 SELECT NOMBRE
                    FROM EMPLEADOS
                    WHERE ID_EMPLEADO=E.SUPERVISOR
              ) AS SUPERVISOR,
              COUNT(*) AS CANTIDAD_SUPERVISADOS
          FROM EMPLEADOS AS E
    ...
    SELECT  /*POR CADA TUPLA*/
          ES.NOMBRE AS SUPERVISOR,
            (
               SELECT  COUNT(*)
                  FROM EMPLEADOS
                  WHERE SUPERVISOR=ES.ID_EMPLEADO
            
            ) AS CANTIDAD_SUPERVISADOS
       FROM EMPLEADOS AS ES
       WHERE ES.ID_EMPLEADO IN (
                                 SELECT  DISTINCT SUPERVISOR
                                   FROM EMPLEADOS
                                   WHERE SUPERVISOR IS NOT NULL
                               );  
        
    /** 1 obtener para todos los empleados
        de  la empresa:
        su nombre y salario
     ...
     SELECT   
              /* DE CADA GRUPO*/
                  ...
                  COUNT(ES.ID_EMPLEADO) AS CANTIDAD_SUPERVISADOS 
          FROM EMPLEADOS AS E LEFT JOIN   EMPLEADOS AS ES
                                ON E.ID_EMPLEADO=ES.SUPERVISOR /* (O,N)*/
   ...
   SELECT    E.NOMBRE AS EMPLEADO,
	...
	SELECT EMPLEADO AS CLAVE_EMPLEADO,
       ...
       /**  3 Obtener para cada empleado: (*TODOS !!*)
...
SELECT E.ID_EMPLEADO AS CLAVE_EMPLEADO,
       ...
       DATEDIFF(CURRENT_DATE(),EP.FECHA_INICIO) AS CANTIDAD_DIAS_TRABAJA
	FROM EMPLEADOS AS E INNER JOIN EMPLEADOS_PROYECTOS AS EP
						 ON EP.EMPLEADO=E.ID_EMPLEADO
	GROUP BY E.ID_EMPLEADO;

           /**   4 PARA CADA PROYECTO DE LA EMPRESA OBTENER,
               SU CLAVE, NOMBRE Y PRESUPUESTO,
               DIAS QUE HACE QUE HA SIDO LANZADO, ( USO FUNCIÓN DATEDIFF)
...
SELECT P.ID_PROYECTO AS CLAVE_PROYECTO,
       ...
       DATEDIFF(CURRENT_DATE(),P.FECHA_INICIO) AS TOTAL_DIAS_QUE_LLEVA
